...
async function ensurePortalCore(env){
 ...
 const migrations=[['full_name','TEXT'],['phone','TEXT'],['business','TEXT'],['city','TEXT'],['username','TEXT'],['password_hash','TEXT'],['password_salt','TEXT'],['status',"TEXT DEFAULT 'pending'"],['created_at','TEXT'],['approved_at','TEXT'],['verification_code','TEXT']];
...
export async function onRequest(context){
 ...
 try{
  ...
  if(path==='/admin/applications'&&method==='GET'){if(!await sessionUser(env,request,'admin'))return json({error:'Unauthorized'},401);const {results:missing}=await env.DB.prepare("SELECT id FROM installers WHERE status='approved' AND (verification_code IS NULL OR verification_code='')").all();for(const row of missing)await env.DB.prepare('UPDATE installers SET verification_code=? WHERE id=?').bind(randomHex(16),row.id).run();const {results}=await env.DB.prepare('SELECT id,full_name,phone,business,city,username,status,created_at,approved_at,verification_code FROM installers ORDER BY CASE status WHEN \'pending\' THEN 0 ELSE 1 END,created_at DESC').all();return json({applications:results})}
  if(path==='/admin/applications'&&method==='POST'){if(!await sessionUser(env,request,'admin'))return json({error:'Unauthorized'},401);const d=await body(request),id=Number(d?.id),action=d?.action;if(!id||!['approve','reject','disable'].includes(action))return json({error:'Invalid action.'},400);const status=action==='approve'?'approved':action==='reject'?'rejected':'disabled',code=status==='approved'?randomHex(16):null;await env.DB.prepare("UPDATE installers SET status=?,approved_at=CASE WHEN ?='approved' THEN datetime('now') ELSE approved_at END,verification_code=CASE WHEN ?='approved' THEN COALESCE(NULLIF(verification_code,''),?) ELSE verification_code END WHERE id=?").bind(status,status,status,code,id).run();if(status!=='approved')await env.DB.prepare("DELETE FROM sessions WHERE role='installer' AND user_id=?").bind(id).run();return json({ok:true,status})}
