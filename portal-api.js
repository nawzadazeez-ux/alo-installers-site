...
export async function onRequest(context){
 ...
 try{
  ...
  if(path==='/calculator/config'&&method==='GET')return json(await calculatorConfig(env),200,{'cache-control':'public, max-age=60'});
  ...
  if(path==='/admin/calculator/item'&&method==='POST'){
   if(!await sessionUser(env,request,'admin'))return json({error:'Unauthorized'},401);await ensureCalculator(env);const d=await body(request),id=Number(d?.id||0),code=clean(d?.code,80).toLowerCase(),category=clean(d?.category,20),quality=clean(d?.quality,20),mode=clean(d?.mode,20),name=clean(d?.name,120),price=Number(d?.price),warranty=clean(d?.warranty,100),watts=Number(d?.watts||0),kw=Number(d?.kw||0),phase=clean(d?.phase,10),maxPanels=Number(d?.maxPanels||0),amps=Number(d?.ampsPerHour||0),sort=Number(d?.sortOrder||0),active=d?.active?1:0;
   if(!/^[a-z0-9._-]+$/.test(code)||!['panel','inverter','battery'].includes(category)||!['high','medium','standard','common'].includes(quality)||!['easy','advanced','both'].includes(mode)||name.length<2||![price,watts,kw,maxPanels,amps,sort].every(Number.isFinite)||price<0)return json({error:'Invalid calculator item.'},400);
