...
export async function onRequest(context){
 ...
 try{
  ...
  if(path==='/admin/applications'&&method==='GET'){if(!await sessionUser(env,request,'admin'))return json({error:'Unauthorized'},401);const {results:missing}=await env.DB.prepare("SELECT id FROM installers WHERE status='approved' AND (verification_code IS NULL OR verification_code='')").all();for(const row of missing)await env.DB.prepare('UPDATE installers SET verification_code=? WHERE id=?').bind(randomHex(16),row.id).run();const {results}=await env.DB.prepare("SELECT i.id,i.full_name,i.phone,i.business,i.city,i.username,i.status,i.created_at,i.approved_at,i.verification_code,COALESCE((SELECT SUM(p.amount) FROM installer_purchases p WHERE p.installer_id=i.id AND p.purchased_at>=datetime('now','-2 months')),0) reward_total FROM installers i ORDER BY CASE i.status WHEN 'pending' THEN 0 ELSE 1 END,i.created_at DESC").all();return json({applications:results})}
